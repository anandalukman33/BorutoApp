...
class SearchHeroesSourceTest {
    ...
    @Test
    fun `Search api with existing hero name, expect empty heroes list, assert LoadResult_Page`() =
        runBlocking {
            val heroSource = SearchHeroesSource(borutoApi = borutoApi, query = "")
            val loadHeroes = heroSource.load(
                PagingSource.LoadParams.Refresh(
                    key = null,
                    loadSize = 3,
                    placeholdersEnabled = false
            ...
            assertTrue { loadHeroes is PagingSource.LoadResult.Page }
        }

    @Test
    fun `Search api with non_existing hero name, assert expect empty heroes list and LoadResult_Page`() =
        runBlocking {
